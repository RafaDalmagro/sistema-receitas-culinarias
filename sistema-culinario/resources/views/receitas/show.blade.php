...
@section('content')
    <div class="container my-5">
        ...
        <div class="d-flex justify-content-end align-items-end">
            ...
            @if (auth()->check() && auth()->id() === $receita->user_id)
                ...
                <form action="/receitas/{{ $receita->id }}" method="POST" class="d-inline">
                    ...
                    <button type="submit" class="btn btn-danger"
                        onclick="return confirm('Tem certeza que deseja excluir essa receita?')">
        ...
        <div class="container my-5">
            <div class="card shadow-sm">
                <div class="card-body">
                    ...
                    <h4 class="text-secondary">Autor</h4>
                    <p class="mb-4">
                        {{ $receita->user->name }}
                    </p>
                    ...
                    <li class="list-group-item">
                        ...
                        @if (auth()->check() && auth()->id() === $comentario->user_id)
                            <button class="btn btn-sm btn-warning"
                                onclick="document.getElementById('edit-form-{{ $comentario->id }}').style.display = 'block';">
                                Editar
                            </button>
                            ...
                            <form action="{{ route('comentarios.destroy', $comentario->id) }}" method="POST"
                                ...
                                <button type="submit" class="btn btn-sm btn-danger"
                                    onclick="return confirm('Tem certeza que deseja excluir este comentário?')">Excluir</button>
                            </form>

                            <form id="edit-form-{{ $comentario->id }}"
                                action="{{ route('comentarios.update', $comentario->id) }}" method="POST"
                                style="display: none;">
                                @csrf
                                @method('PUT')
                                <div class="mb-3">
                                ...
                                <button type="button" class="btn btn-sm btn-secondary"
                                    onclick="document.getElementById('edit-form-{{ $comentario->id }}').style.display = 'none';">Cancelar</button>
                            </form>
                        @endif
                    </li>
                @endforeach
